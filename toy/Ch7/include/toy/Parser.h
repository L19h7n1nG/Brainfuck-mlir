...
class Parser {
   public:
    ...
    std::unique_ptr<ModuleAST> parseModule() {
        ...
        // Parse functions and structs one at a time and accumulate in this
        // vector.
   ...
   private:
    ...
    std::unique_ptr<ExprAST> parseTensorLiteralExpr() {
        ...
        if (values.empty())
            return parseError<ExprAST>(
                    "<something>", "to fill literal expression");
        lexer.getNextToken(); // eat ]
        ...
        if (llvm::any_of(values, [](std::unique_ptr<ExprAST>& expr) {
            ...
            auto* firstLiteral =
                    llvm::dyn_cast<LiteralExprAST>(values.front().get());
            if (!firstLiteral)
                return parseError<ExprAST>(
                        "uniform well-nested dimensions",
            ...
            // Sanity check that shape is uniform across all elements of the
            // list.
    ...
    std::unique_ptr<ExprAST> parseStructLiteralExpr() {
        ...
        do {
            ...
            if (lexer.getCurToken() == '[') {
                ...
                if (lexer.getCurToken() != '{')
                    return parseError<ExprAST>(
                            "{, [, or number", "in struct literal expression");
            ...
            if (lexer.getCurToken() != ',')
                return parseError<ExprAST>(
                        "} or ,", "in struct literal expression");

            lexer.getNextToken(); // eat ,
        } while (true);
        if (values.empty())
            return parseError<ExprAST>(
                    "<something>", "to fill struct literal expression");
        lexer.getNextToken(); // eat }

        return std::make_unique<StructLiteralExprAST>(
                std::move(loc), std::move(values));
    }
    ...
    std::unique_ptr<ExprAST> parseCallExpr(
        ...
        if (name == "print") {
            if (args.size() != 1)
                return parseError<ExprAST>(
                        "<single arg>", "as argument to print()");
        ...
        return std::make_unique<CallExprAST>(
                loc, std::string(name), std::move(args));
    }
    ...
    /// Recursively parse the right hand side of a binary expression, the
    /// ExprPrec argument indicates the precedence of the current binary
    /// operator.
    ///
    /// binoprhs ::= ('+' primary)*
    std::unique_ptr<ExprAST> parseBinOpRHS(
        ...
        while (true) {
            ...
            if (!rhs)
                return parseError<ExprAST>(
                        "expression", "to complete binary operator");
    ...
    std::unique_ptr<VarDeclExprAST> parseTypedDeclaration(
        ...
        if (requiresInitializer) {
            if (lexer.getCurToken() != '=')
                return parseError<VarDeclExprAST>(
                        "initializer", "in variable declaration");
            lexer.consume(Token('='));
            expr = parseExpression();
        }
    ...
    /// Parse a variable declaration, for either a tensor value or a struct
    /// value, with an optionally required initializer. decl ::= var identifier
    /// [ type ] (= expr)? decl ::= identifier identifier (= expr)?
    std::unique_ptr<VarDeclExprAST> parseDeclaration(bool requiresInitializer) {
        ...
        if (lexer.getCurToken() != tok_identifier)
            return parseError<VarDeclExprAST>(
                    "type name", "in variable declaration");
        auto        loc = lexer.getLastLocation();
    ...
    std::unique_ptr<VarDeclExprAST> parseVarDeclaration(
            bool requiresInitializer) {
        if (lexer.getCurToken() != tok_var)
        ...
        if (lexer.getCurToken() != tok_identifier)
            return parseError<VarDeclExprAST>(
                    "identified", "after 'var' declaration");
        std::string id(lexer.getId());
        lexer.getNextToken(); // eat id
    ...
    std::unique_ptr<PrototypeAST> parsePrototype() {
        ...
        if (lexer.getCurToken() != ')') {
            do {
                ...
                args.push_back(std::make_unique<VarDeclExprAST>(
                        std::move(loc), name, type));
                if (lexer.getCurToken() != ',')
                    break;
                lexer.consume(Token(','));
                if (lexer.getCurToken() != tok_identifier)
