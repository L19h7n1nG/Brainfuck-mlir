...
struct ConstantOpLowering : public OpRewritePattern<toy::ConstantOp> {
    ...
    LogicalResult matchAndRewrite(toy::ConstantOp op, PatternRewriter& rewriter)
            const final {
        ...
        if (!valueShape.empty()) {
            for (auto i : llvm::seq<int64_t>(
                         0,
                         *std::max_element(
                                 valueShape.begin(), valueShape.end())))
...
struct ReturnOpLowering : public OpRewritePattern<toy::ReturnOp> {
    ...
    LogicalResult matchAndRewrite(toy::ReturnOp op, PatternRewriter& rewriter)
            const final {
