   ...
   public:
    ...
    mlir::ModuleOp mlirGen(ModuleAST& moduleAST) {
        ...
        for (auto& record : moduleAST) {
            if (FunctionAST* funcAST =
                        llvm::dyn_cast<FunctionAST>(record.get())) {
                mlir::toy::FuncOp func = mlirGen(*funcAST);
                if (!func)
                    return nullptr;
                functionMap.insert({func.getName(), func});
            } else if (
                    StructAST* str = llvm::dyn_cast<StructAST>(record.get())) {
   ...
   private:
    ...
    mlir::LogicalResult mlirGen(StructAST& str) {
        ...
        structMap.try_emplace(
                str.getName(), StructType::get(elementTypes), &str);
        return mlir::success();
    }
    ...
    mlir::toy::FuncOp mlirGen(PrototypeAST& proto) {
        ...
        return builder.create<mlir::toy::FuncOp>(
                location, proto.getName(), funcType);
    }
    ...
    mlir::toy::FuncOp mlirGen(FunctionAST& funcAST) {
        ...
        for (const auto nameValue :
            ...
            if (failed(declare(
                        *std::get<0>(nameValue), std::get<1>(nameValue))))
                return nullptr;
        }
        ...
        // expression (this would possibly help the REPL case later)
        ReturnOp returnOp;
        if (!entryBlock.empty())
    ...
    /// Return the struct type that is the result of the given expression, or
    ...
    std::optional<size_t> getMemberIndex(BinaryExprAST& accessOp) {
        ...
        VariableExprAST* name =
                llvm::dyn_cast<VariableExprAST>(accessOp.getRHS());
        if (!name)
            return std::nullopt;
    ...
    mlir::Value mlirGen(BinaryExprAST& binop) {
        // First emit the operations for each side of the operation before
        // emitting the operation itself. For example if the expression is `a +
        // foo(a)` 1) First it will visiting the LHS, which will return a
        // reference to the
        //    value holding `a`. This value should have been emitted at
        //    declaration time and registered in the symbol table, so nothing
        //    would be codegen'd. If the value is not in the symbol table, an
        //    error has been emitted and nullptr is returned.
        // 2) Then the RHS is visited (recursively) and a call to `foo` is
        // emitted
        //    and the result value is returned. If an error occurs we get a
        //    nullptr and propagate.
        //
    ...
    /// Emit a return operation. This will return failure if any generation
    /// fails.
    ...
    mlir::DenseElementsAttr getConstantAttr(LiteralExprAST& lit) {
        ...
        // The type of this attribute is tensor of 64-bit floating-point with
    ...
    mlir::DenseElementsAttr getConstantAttr(NumberExprAST& lit) {
        ...
        return mlir::DenseElementsAttr::get(
                dataType, llvm::ArrayRef(lit.getValue()));
    }
    ...
    mlir::Value mlirGen(LiteralExprAST& lit) {
        ...
        // Build the MLIR op `toy.constant`. This invokes the
    ...
    mlir::Value mlirGen(CallExprAST& call) {
        ...
        if (callee == "transpose") {
            ...
            if (call.getArgs().size() != 2) {
                mlir::emitError(location, "mm operator in a binary op");
                return nullptr;
            }
    ...
    /// Handle a variable declaration, we'll codegen the expression that forms
    /// the initializer and record the value in the symbol table before
    ...
    mlir::Value mlirGen(VarDeclExprAST& vardecl) {
        ...
        if (!varType.name.empty()) {
            ...
            // Otherwise, we have the initializer value, but in case the
            // variable was declared with specific shape, we emit a "reshape"
            // operation. It will get optimized out later as needed.
        } else if (!varType.shape.empty()) {
            value = builder.create<ReshapeOp>(
                    loc(vardecl.loc()), getType(varType.shape), value);
        }
    ...
    /// Codegen a list of expression, return failure if one of them hit an
    /// error.
