...
struct ToyInlinerInterface : public DialectInlinerInterface {
    ...
    /// Handle the given inlined terminator(toy.return) by replacing it with a
...
void ConstantOp::print(mlir::OpAsmPrinter& printer) {
    ...
    printer.printOptionalAttrDict(
            (*this)->getAttrs(), /*elidedAttrs=*/{"value"});
    printer << getValue();
}
...
void FuncOp::print(mlir::OpAsmPrinter& p) {
    // Dispatch to the FunctionOpInterface provided utility method that prints
    // the function operation.
...
mlir::LogicalResult ReturnOp::verify() {
    // We know that the parent operation is a function, because of the
    // 'HasParent' trait attached to the operation definition.
...
void MatMulOp::inferShapes() {
    auto lhsShpape =
            getOperand(0).getType().cast<RankedTensorType>().getShape();

    auto rhsShape = getOperand(1).getType().cast<RankedTensorType>().getShape();

    llvm::SmallVector<int64_t, 2> res{lhsShpape[0], rhsShape[1]};

    getResult().setType(RankedTensorType::get(
            res,
            getOperand(0).getType().cast<RankedTensorType>().getElementType()));
}

mlir::LogicalResult MatMulOp::verify() {
    auto getIthElemType = [&](int idx) {
        return getOperand(idx).getType().cast<TensorType>().getElementType();
    };

    if (getIthElemType(0) != getIthElemType(1)) {
        emitError()
                << "MM ops operands should have the same element type. Curr Lhs: "
                << getIthElemType(0) << " Rhs: " << getIthElemType(1);
    }
...
void MatMulOp::build(
        ...
        Value                   lhs,
        Value                   rhs) {
    odsState.addTypes(UnrankedTensorType::get(
            lhs.getType().cast<TensorType>().getElementType()));
...
struct StructTypeStorage : public mlir::TypeStorage {
    ...
    /// storage instance. This type will be used when uniquing an instance of
    /// the type storage. For our struct type, we will unique each instance
    /// structurally on the elements that it contains.
    ...
    /// we haven't already uniqued an instance of the given key.
    ...
    /// parameters. These parameters will be provided when constructing the
    /// storage instance itself. Note: This method isn't necessary because KeyTy
    /// can be directly constructed with the given parameters.
    static KeyTy getKey(llvm::ArrayRef<mlir::Type> elementTypes) {
        return KeyTy(elementTypes);
    }

    /// Define a construction method for creating a new instance of this
    /// storage. This method takes an instance of a storage allocator, and an
    /// instance of a `KeyTy`. The given allocator must be used for *all*
    /// necessary dynamic allocations used to create the type storage and its
    /// internal.
...
void ToyDialect::printType(mlir::Type type, mlir::DialectAsmPrinter& printer)
        const {
